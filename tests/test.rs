...
fn block_sync_test() {
    ...
    let mut init_nodes: Vec<Node> = keypairs
        .into_iter()
        .zip(network_stubs)
        .map(|(keypair, network)| Node::new(keypair.clone(), network, init_as.clone(), init_vs_updates.clone()))
        .collect();
    ...
    let last_node = Node::new(last_keypair[0].clone(), last_newtork[0].clone(), init_as.clone(), init_vs_updates.clone());
