    ...
    fn execute(mut self) {
        ...
        loop {
            ...
            if self.hotstuff.is_view_outdated(view_info) {
                self.hotstuff.enter_view(view_info.clone(), &mut self.block_tree, &mut self.app).expect("HotStuff failure!")
            }
            ...
            match self.pm_stub.recv(self.chain_id, view_info.view, view_info.deadline) {
                Ok((origin, msg)) => {
                    match msg {
                        ...
                        ProgressMessage::BlockSyncTriggerMessage(msg) => self.block_sync_client.on_receive_msg(msg, &origin, &self.block_tree).expect("Block Sync Client failure!"),
                    }
                },
